...
Namespace DGVColumnSelector
	Class MenuControl
		...
		Private Class MenuCommand
			...
			Public Sub New(csText As String, bChecked As Boolean)
				Me.New(csText, bChecked, False)
			End Sub
			Public Sub New(csText As String)
				Me.New(csText, False, False)
			End Sub
			Public Sub New(csText As String, bChecked As Boolean, bDone As Boolean)
				m_csText = csText
				'm_iIndex = iIndex;
				m_bChecked = bChecked
				m_bDone = bDone
		...
		Public Function ChangeChecked(iIndex As Integer, g As Graphics) As Boolean
			m_pMenuCommands(iIndex).Checked = Not m_pMenuCommands(iIndex).Checked
			Draw(g)
			Return m_pMenuCommands(iIndex).Checked
		End Function

		Public Sub Add(csText As String, bChecked As Boolean)
			m_pMenuCommands.Add(New MenuCommand(csText, bChecked))
		End Sub

		Public Sub Prepare(g As Graphics)
		...
		Private Function HitTest(X As Integer, Y As Integer) As MenuCommand
		...
		Public Function HitTestMouseMove(X As Integer, Y As Integer) As Boolean
			Dim pMenuCommand As MenuCommand = HitTest(X, Y)
			If pMenuCommand IsNot m_pTracMenuItem Then
			...
			End If
		End Function
		Public Function HitTestMouseDown(X As Integer, Y As Integer) As Boolean
			Dim pMenuCommand As MenuCommand = HitTest(X, Y)
		...
		Private Sub DrawBackground(g As Graphics, rectWin As Rectangle)
		...
		Private Sub DrawAllCommands(g As Graphics)
			Dim iTop As Integer = 2
			For Each pMenuCommand As MenuCommand In m_pMenuCommands
				DrawSingleCommand(g, iTop, pMenuCommand, pMenuCommand Is m_pTracMenuItem)
			Next
		End Sub

		Private Sub DrawSingleCommand(g As Graphics, ByRef iTop As Integer, pMenuCommand As MenuCommand, hotCommand As Boolean)
