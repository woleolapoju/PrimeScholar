...
errhdl:
    ...
    Private Sub LoadClientBalances()
        ...
        If RadSchool.Checked = True Then
            ...
            cmSQL.CommandText = "SELECT Sn, Name, 0 AS CR, SUM(Amount) AS DR,0-SUM(Amount) AS Balance, [Currency] AS TheCurrency FROM Ledger WHERE Source='SCHOOL' " & IIf(strQry1 = "", "", " AND " + strQry1) & " GROUP BY Sn, Name,Currency"
        ElseIf RadStudent.Checked = True Then
            If cboCountry.Text <> "ALL" Then strQry1 = " SchCountry ='" & cboCountry.Text & "'"
            If cSchool.Text <> "ALL" Then strQry1 = strQry1 + IIf(strQry1 = "", " ", " AND ") + " SchName ='" & cSchool.Text & "'"
            cmSQL.CommandText = "SELECT Sn, Name, 0 AS CR, SUM(Amount) AS DR,0-SUM(Amount) AS Balance, [Currency] AS TheCurrency FROM Ledger WHERE Source='STUDENT' " & IIf(strQry1 = "", "", " AND " + strQry1) & " GROUP BY Sn, Name,Currency"
        ElseIf RadStaff.Checked = True Then
            If cboCountry.Text <> "ALL" Then strQry1 = " SchCountry ='" & cboCountry.Text & "'"
            cmSQL.CommandText = "SELECT Sn, Name, 0 AS CR, SUM(Amount) AS DR,0-SUM(Amount) AS Balance, [Currency] AS TheCurrency FROM Ledger WHERE Source='STAFF' " & IIf(strQry1 = "", "", " AND " + strQry1) & " GROUP BY Sn, Name,Currency"
        Else 'RadStake.Checked = True
            If cboCountry.Text <> "ALL" Then strQry1 = " SchCountry ='" & cboCountry.Text & "'"
            cmSQL.CommandText = "SELECT Sn, Name, 0 AS CR, SUM(Amount) AS DR,0-SUM(Amount) AS Balance, [Currency] AS TheCurrency FROM Ledger WHERE Source='STAKEHOLDERS' " & IIf(strQry1 = "", "", " AND " + strQry1) & " GROUP BY Sn, Name,Currency"
...
handler:
    ...
    Private Sub DGrid_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles DGrid.DoubleClick
        ...
        ChildForm.cboMandateNo.Text = DGrid.Item(0, DGrid.Tag).Value
        ChildForm.MandateNo = DGrid.Item(0, DGrid.Tag).Value
        ChildForm.dIndex = -1 ' DGrid.Item("dIndex", DGrid.Tag).Value
